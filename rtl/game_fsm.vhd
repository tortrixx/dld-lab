...
begin
    ...
    --    ⚠️ 选择表达式是 5 位，每个 choice 必须也是 5 位。
    -- ============================================================
    process (i_key_code)
    begin
        -- 默认值先行，防 latch
    ...
    --    ⚠️ 不能把 i_tick_2hz 直接当方波送出去：本项目所有 tick
    --       都是**单周期使能**（一拍宽），把它当方波得到的是 1Hz。
    --    ⚠️ 用"翻转"而不是"数到一半置 1"：只花 1 个触发器表达状态本身。
    -- ============================================================
    process (clk)
    begin
        if rising_edge(clk) then
    ...
    --    ⚠️ r_cnt_tens = 0 且 r_cnt_ones = 0 时必须**保持不变**，
    ...
    -- ============================================================
    -- ⑪ 状态类输出的接线
    --    ⚠️ o_game_cnt_bcd 的拼接顺序是**生产侧的契约**：十位在 (7..4)、
    ...
    -- ============================================================
    -- ⑫ 提高要求②：多图案随机（第二关在上箭头 / 下箭头里二选一）
    --    ⚠️ 用**寄存输出**（时序分析实测：组合版会把
