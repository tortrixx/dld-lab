...
--      · 4 块 × 8 行 = 32 拍刷新一帧（约 0.64µs @50MHz），点阵 500Hz 扫描看不出来；
--      · 引擎在**事件**（开局 / 散落落子 / 选择 / 移动 / 锁定）时启动一帧，
--        完成后静止 → 显示的是稳定整帧，不会撕裂。
--    代价：o_px_red / o_px_green 由"纯组合"变为**寄存输出**；
--    且 o_solved 在刷新期间（≤0.64µs）保持 '0'（这是必需的，见下面 ⑧ 的说明）。
--
--  ⭐【散落的重叠判定：用包围盒，不用掩码求交】
--    原方案"算候选位置的 64 位掩码再与已放置并集求交"，需要再造一份平移逻辑。
--    现改为**包围盒不相交**判据：
--        相交 ⟺ (r1 < r2+h2) 且 (r2 < r1+h1) 且 (c1 < c2+w2) 且 (c2 < c1+w1)
...
architecture rtl of puzzle_ctrl is
    ...
    --   ⚠️ 寄存带来的 1 拍延迟不影响功能：散落序列器要在 o_round_start
...
begin
    ...
    -- ============================================================
    -- ② 行扫描引擎的输入选择（组合）
    --    ⚠️ 零片的相对掩码只在前 3 行 × 前 3 列有非零位（docs/02 §8.3），
    ...
    -- 重叠判定（保守 · 包围盒不相交）
    --   候选块 (行,列,高,宽) 与已放置的第 q 块，包围盒相交 ⟺ 四个不等式同时成立。
    --   ⚠️ 只比较"已放置"的前 r_piece 块；不能把还没摆的块算进去
    ...
    --   代价：每次尝试从 2 拍变 3 拍 → 48 次尝试 × 3 拍 ≈ 2.9µs，远小于 5 秒预览。
    process (clk)
    begin
        if rising_edge(clk) then
            if rst = '1' then
    ...
    --    ⚠️ 不能压成两拍：① rng_lfsr 在 i_step 的那个时钟沿才更新，"同一拍读 i_rnd"
    --       读到的是旧值（§9.5 那个"每次同值"的坑会重现）；② 判据必须寄存，
    --       否则 piece_rom → 包围盒比较 → 序列器 会连成一条 20.4ns 的组合链。
    --    上界：4 块 ×（8 次随机 + 4 次回退）× 3 拍 = 144 拍 ≈ 2.9µs @50MHz，
    ...
    --    ⚠️ 判据用 >= 而不是 =：写 = 时，一个陈旧的 v_next = 3 在 v_last = 2 时
    --       （第二关打完回到第一关）会跳进 else 分支算出 4 → 越界。
    --    ⚠️ 不用 mod：piece_count 是信号，mod 会综合出一个除法器。
    -- ============================================================
    process (clk)
