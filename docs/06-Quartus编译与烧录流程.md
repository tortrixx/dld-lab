...
| 烧 `puzzle_top`、拨 SW7=1 后**看不到**"点阵全黄闪 + 数码管全 8 闪"，**直接就是待机**（DISP7 显 "5"） | ⭐ **正常**：开机自检**只持续 2 秒**（**要求 2 明文**"2 秒后系统进入待机状态"；`T_SELFTEST = 2`），**不是没实现** | **不是故障，是那 2 秒过去了**。① **先盯住板子再拨 SW7**；② 或**按住最右按键（BTN0）再松开**；③ 或**录视频回放前 3 秒**。见 `ERRORS.md` ERR-0034 |
