...
> # ⚠️ 切顶层 = **换掉板上跑的程序**，不是"烧一次新版本"
>
> **四个显示器件与键盘的含义全部改变**（`ld` 只在自检顶层存在；自检顶层没有 `S_SELF_TEST`、
> 没有游戏逻辑、没有 `puzzle_ctrl`）。
> **动手前必须先跟现场的人说清"接下来会看到什么"** —— 否则现场只能拿旧画面的记忆去比，**必然误判成"坏了"**
> （2026-09-24 实测：切到自检顶层后现场报"没有自检、都乱闪烁"，见 `ERRORS.md` **ERR-0036**）。
>
> **两个顶层的期望画面对照（切完先看这一张）**：
>
> | | `board_test_top`（硬件自检） | `puzzle_top`（整机拼图） |
> |---|---|---|
> | **开机** | **9 阶段自动轮播**（每阶段 2 秒，共 36 秒一圈） | **2 秒开机自检** → 待机 |
> | **点阵** | 逐行 / 逐列红 / 逐列绿 / 全黄 | 自检：全黄 2Hz 闪；预览：目标图案；游戏中：零片 |
> | **数码管** | DISP7 显**阶段号**（阶段 5/6/7 例外，另有内容） | 待机：`5 … 1`；预览：5 秒倒数 |
> | **16 个 LED** | **有**（`ld[15:8]`=阶段号、`ld[7:0]`=**子步号，每 0.25 秒变**） | **没有这个端口** —— 不受本设计驱动 |
> | **按键** | 只有 **BTN0** 有用（阶段 9）；键盘只在阶段 7 用 | 键盘全部可用（开始/选择/方向/确认） |
> | **SW7=0** | 数码管/点阵/蜂鸣器全灭，**但 LD 不停在阶段 1**（只有 LD8 亮） | 数码管/点阵/蜂鸣器全灭 |
> | **看起来像什么** | ⚠️ **"乱"是正常的** —— LD 低 8 位每 0.25 秒跳一次、数码管每 2 秒跳一次 | 自检 2 秒 → 静态待机 |
>
> ⭐ **一句话**：**"2 秒自检"只属于 `puzzle_top`；看到"乱闪 + LD 在动"就是 `board_test_top`。**
...
| ⭐ 烧 `puzzle_top` 后**看不到自检**，而 DISP7 **从 5 开始倒数**、**点阵自己亮出图案** | ⚠️ **这不是待机态！** 待机 `S_IDLE` 的 "5" 是**静态**的；**会"计时"的只有 `S_PREVIEW`（预览 5 秒倒计时）**。而进预览的唯一非按键路径不存在 —— 说明有一个**幻影「开始」按键**把系统踢出了自检态 | 根因 = **键盘扫描极性**（`ERRORS.md` **ERR-0035**，已改 `keypad_scan` 的 `KP_ACTIVE` 为低有效）。**判别口径**：**待机时点阵应当全灭**；若点阵自己亮出图案 → 幻影按键。⚠️ 现场描述里的"**计时 / 倒数**"这类词，就是状态线索 |
