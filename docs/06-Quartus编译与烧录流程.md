...
6. 把 **SW7 拨到 "1"（开）** —— 现象**取决于当前烧进去的是哪个顶层**，
   ...
   - 顶层 = **`board_test_top`**（第一次进实验室默认烧这个）：进入 **9 阶段自动轮播自检**，
     每阶段约 2 秒，`ld[15:8]` 显示当前阶段号（1~9），阶段 7 的键盘项停留无限期。
   ...
   > 的输入，同时门控顶层所有输出 —— 拨到 0 则全灭、蜂鸣器无声，这是**正常**的），
...
| 数码管/点阵全灭，LED 也全灭 | **SW7 在 "0"**（它是 `clk_gen.sys_en` 的输入，同时门控顶层所有输出） | 拨到 **"1"**。这是**正常设计**，不是故障 |
