...
| **16 个灯全都在动**，找不到"`LD8` 常亮 + `LD0~LD7` 齐闪" | ⭐ **正常，且是大概率画面**：**阶段 1 只占 36 秒里的 2 秒（≈5.6%）**，其余 34 秒是阶段 2~9（`ld[15:8]`=阶段号、`ld[7:0]`=子步号）→ 16 个灯本来就都在动 | **不是故障，是"没撞上阶段 1"**。看**最左边那位数码管 DISP7** 的读数（= 当前阶段号）；或**松开最右按键后的 2 秒内**看。见下方「30 秒判据」与 `ERRORS.md` ERR-0031 |
